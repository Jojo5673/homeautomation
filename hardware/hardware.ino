...
uint8_t passcode = 1; 
uint8_t digit1Val = 0, digit2Val = 0, digit3Val = 0, digit4Val = 0; 
uint16_t potValue = 0;
int mappedValue = 0;
...
void loop() {
  ...
  int normalized_value = map(potValue, 0, 4095, 0, 9);

  // Assign the mapped value to the currently selected digit
  switch (currentDigit) {
      case 1:
          digit1(normalized_value);
          digit1Val=normalized_value;
          break;
      case 2:
          digit2(normalized_value);
          digit2Val=normalized_value;
          break;
      case 3:
          digit3(normalized_value);
          digit3Val=normalized_value;
          break;
      case 4:
          digit4(normalized_value);
          digit4Val=normalized_value;
          break;
      default:
          break;
  }
...
void digit1(uint8_t number){
  ...
  tft.fillRoundRect(5, 260, 50, 50, 5, ILI9341_MAROON);
...
void digit2(uint8_t number){
  ...
  tft.setFont(&FreeSansBold18pt7b);
  // 2. Draw a filled rounded rectangle close to the bottom of the screen. Give it any colour you like 
  tft.fillRoundRect(65, 260, 50, 50, 5, ILI9341_MAROON);
  // 3. Set cursor to the appropriate coordinates in order to write the number in the middle of the box 
...
void digit3(uint8_t number){
  ...
  tft.setFont(&FreeSansBold18pt7b);
  // 2. Draw a filled rounded rectangle close to the bottom of the screen. Give it any colour you like
  tft.fillRoundRect(125, 260, 50, 50, 5, ILI9341_MAROON);
  // 3. Set cursor to the appropriate coordinates in order to write the number in the middle of the box 
...
void digit4(uint8_t number){
  ...
  tft.setFont(&FreeSansBold18pt7b);
  // 2. Draw a filled rounded rectangle close to the bottom of the screen. Give it any colour you like
  tft.fillRoundRect(185, 260, 50, 50, 5, ILI9341_MAROON);
  // 3. Set cursor to the appropriate coordinates in order to write the number in the middle of the box 
